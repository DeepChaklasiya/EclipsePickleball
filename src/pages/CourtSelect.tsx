...
const CourtSelect = () => {
  ...
  const handleCourtSelect = (courtId: string) => {
    if (courtId === selectedCourtId || isLocationComingSoon) return;
  ...
  const formatBookingSummary = () => {
    ...
    return `${booking.location.name} • ${format(booking.date, "MMM d")} • ${booking.timeSlot.startTime}-${booking.timeSlot.endTime}`;
  };
  ...
  return (
    <div className="page-container">
      <div className="content-with-sticky-button">
        ...
        <div className="mb-8">
          <div className="text-sm font-medium bg-gradient-to-r from-pink-300 to-amber-200 bg-clip-text text-transparent mb-2">Current Selection</div>
        ...
        {isLocationComingSoon ? (
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <h2 className="text-2xl font-bold text-center mb-3">Coming Soon!</h2>
            <p className="text-center text-muted-foreground max-w-md">
              Eclipse Andromeda is under construction. Our newest cosmic courts will be opening soon!
            </p>
            <div className="mt-8 flex flex-col items-center">
              <p className="text-sm text-muted-foreground mb-2">Please check back later</p>
              <Button
                onClick={() => navigate(-1)}
                variant="cosmic"
                size="sm"
          ...
          <>
            {/* <h3 className="text-center font-medium bg-gradient-to-r from-pink-300 to-amber-200 bg-clip-text text-transparent mb-6">SELECT YOUR COSMIC COURT</h3> */}
                ...
                <div className="flex flex-col items-center w-full">
                  ...
                  <div className="flex flex-col space-y-5 py-4 w-full items-center justify-center">
                    {courts.map((court, index) => (
                      <div 
                        key={court.id} 
                        className="cursor-pointer transition-all duration-300 hover:scale-[1.02] flex justify-center items-center w-full"
                        onClick={() => handleCourtSelect(court.id)}
                      >
                        <div className={`flex flex-row items-center p-2 rounded-lg transition-all duration-300 w-full justify-center ${
                          ...
                          <div className="relative">
                            ...
                            <div className="relative">
                              ...
                              <div className="absolute top-1 left-1/2 transform -translate-x-1/2 bg-gradient-to-r from-pink-400/90 to-amber-300/90 text-black text-sm font-bold py-1 px-4 rounded-full border border-white whitespace-nowrap">
            ...
            <h3 className="text-center font-medium text-white mb-4">Selected Court: {selectedCourtId ? courts.find(c => c.id === selectedCourtId)?.name : 'None'}</h3>
          </>
        )}
      </div>
      ...
      {showBuffer && (
        <div className={`fixed inset-0 bg-black/70 flex flex-col items-center justify-center z-50 ${bufferComplete ? 'buffer-fade-out' : 'buffer-fade-in'}`}>
          <div className="planet-bounce">
            <div className="w-20 h-20 relative">
              ...
              {bufferPlanet.name === "Earth" && (
                <div className="absolute -top-1 -right-1 planet-spin" style={{ animationDuration: '4s' }}>
          ...
          <div className="mt-6 text-white text-center">
            <p className="text-xl font-bold">Preparing {bufferPlanet.name} Court</p>
            <p className="text-sm text-white/70 mt-1">Please wait a moment...</p>
          </div>
        </div>
      )}
